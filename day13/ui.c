...
void enter_ui(struct Graphic *graphic, struct MemMan *memman) {
	...
	drawmouse(&mouselayer->graphic, COL_D_RED);

	mouselayer->y = 100;
	mouselayer->x = 100;

	layer_setzindex(ctl, mouselayer, 2);
...
void drawint(int value, int x, int y) {
	char buf[30];
    itoa(buf, value);
    drawtext(ctl->graphic, COL_RED, x, y, buf);
}
...
void handle_mouse(struct MouseDec *mousedec) {
    ...
    if (mousex > scrnx - MOUSE_WIDTH) {
        mousex = scrnx - MOUSE_WIDTH;
    }
    if (mousey < 0) {
        mousey = 0;
    }
    if (mousey > scrny - MOUSE_HEIGHT) {
        mousey = scrny - MOUSE_HEIGHT;
    }
