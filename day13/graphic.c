...
static void drawchar(struct Graphic *graphic, char color,
...
void drawtext(struct Graphic *graphic, unsigned char color, int x, int y, char *text) {
	while (*text) {
        drawchar(graphic, color, x, y, font + *text * 16);
        text ++;
        x += 8;
    }
}
...
static void drawchar(struct Graphic *graphic, char color,
    ...
    for (int i = 0; i < 16; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            if (data & (1 << j)) {
				setpixel(graphic, color, x + j, y + i);
            }
        }
    }
}
